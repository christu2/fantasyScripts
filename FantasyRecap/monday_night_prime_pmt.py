#!/usr/bin/env python3
"""
BFL Monday Night Prime (PMT Barstool Edition)
=============================================
A hilarious, profane, high-energy 8-to-10 minute post-game podcast show in the style
of Pardon My Take (PMT):
- Hosts: Big Cat style (ChristopherNeural) & PFT style (BrianMultilingualNeural)
...
VOICE_HOST1 = 'en-US-ChristopherNeural'       # Lead Anchor (Big Cat vibe)
VOICE_HOST2 = 'en-US-BrianMultilingualNeural' # Color Analyst / Roast Master (PFT vibe)

# Strict phonetic substitutions for natural TTS speech
PHONETIC_RULES = [
...
def build_pmt_dialogue(season: int = 2025, week_num: int = 1) -> list:
    """
    Constructs an extensive, funny, unfiltered 8-to-10 minute PMT-style podcast script.
    """
    d = []

    # ACT 1: COLD OPEN & KICKOFF
    d.append(('CHRIS', f"What is up everybody! Welcome to BFL Monday Night Prime, presented by the BFL Podcast Network. The Monday night clock just struck double zeroes, week {week_num} of the {season} season is in the books, and fantasy football is officially back to ruining our mental health. I'm Chris alongside Dave, and holy shit Dave, what an absolute circus of an opening week!"))
    d.append(('DAVE', "Chris, I am emotionally exhausted and it's only week one! Commissioner Nick Christus has this eighteen-year-old league operating at peak degeneracy. We had heartbreaking bad beats, thirty-point public executions, elite wide receivers putting up actual goose eggs, and coaching decisions so criminally stupid they should be investigated by the FBI!"))
    ...
    d.append(('DAVE', "Are you kidding me, Chris?! Emelie got completely robbed by the fantasy gods, but mostly by her own damn lineup sheet! She starts Nico Collins, and the dude puts up four points on a fourteen-point projection. Meanwhile, Adam gets a twenty-three-point bailout from Bijan Robinson to save his ass because Joe Burrow looked like he was playing with a blindfold on, scoring under nine points."))
    d.append(('CHRIS', "And Dave, look at Emelie's bench! She had Wan'Dale Robinson rotting on her pine with fourteen and a half points! If she makes the simple swap and benches Nico for Wan'Dale, she wins by seven points and is celebrating with a drink. Instead, she loses by three point zero four points. That is pure, unfiltered pain."))
    d.append(('DAVE', "That is a sickening beat! Leaving points on the bench when you lose by three points is the kind of mistake that makes you stare at your ceiling until four in the morning questioning all your life choices. Adam walks out with a greasy win, and Emelie takes the toughest L of week one."))

    # ACT 2: SYDNEY DESTROYS LUKOSE & THE 4-TIME GOAT
    d.append(('CHRIS', "Now let's head over to the East Division for the absolute demolition of the week. Sydney walked into Luke-ose's house, kicked his front door down, and delivered a one hundred and one to sixty-nine ass-kicking!"))
    d.append(('DAVE', "Sydney was an absolute savage, Chris! She got twenty-five points from Zay Flowers, who went totally nuclear, and her squad looked like a legitimate championship juggernaut. But can we talk about Luke-ose for a second? The four-time champion had the single biggest coaching brain-fart I have seen in eighteen years of BFL history!"))
    d.append(('CHRIS', "It was a total disaster, Dave. Luke-ose started Drake Maye, who gave him fifteen points, while Justin Fields was chilling on his bench dropping nearly thirty fantasy points! A fourteen-point quarterback blunder! Kenneth Walker laid an egg with under four points, and Sydney just ran him out of the gym by thirty-one points."))
    d.append(('DAVE', "Sydney proved she is the undisputed queen of the West Division and put the entire league on notice. Luke-ose has four championship rings in his trophy case, but after getting embarrassed like that on opening week, he's holding an eight A M team meeting to find out who forgot to set the lineup!"))
    ...
    d.append(('CHRIS', "Over in the North Division, our fearless Commissioner Nick and the MEE-ko-nos Minotaurs took care of business, handling Dino one hundred to eighty-five!"))
    ...
    d.append(('CHRIS', "Down in the cross-division matchup, Nile rolled over Sum-rahn one hundred and two to eighty-five behind a twenty-eight-point masterclass from Justin Herbert."))
    d.append(('DAVE', "Justin Herbert was dropping dimes all day! Sum-rahn got twenty-one from Emeka Egbuka, but Jerome Ford gave him literally one single point in his backfield. You cannot win in this league getting one point from your running back, period."))
    ...
    d.append(('CHRIS', "Now let's head down to the media room for our post-game press conference soundbites. Sydney was asked about taking down Luke-ose and said: 'Dropping a thirty-point beatdown on the four-time champ in week one sets the standard. We are here to win the whole damn thing!'"))
    ...
    d.append(('CHRIS', "Meanwhile, Luke-ose was pissed off in the hallway, telling our reporters: 'Disaster across the board. Starting Drake Maye over Justin Fields cost us fourteen points. We are holding an emergency meeting at eight A M to fix this shit.'"))
    ...
    d.append(('DAVE', "Get your waiver bids in tonight, bench the bums who gave you zero points, and for the love of God, don't leave thirty points on your pine like Luke-ose! For Chris, Dave, and the entire BFL Monday Night Prime crew, have a great night everybody!"))

    return d

async def produce_pmt_show(season: int = 2025, week_num: int = 1, post_to_discord: bool = True):
    print("\n" + "="*75)
    print(f"🎙️ BFL MONDAY NIGHT PRIME (PMT EDITION): WEEK {week_num} ({season})")
    print("="*75)

    dialogue = build_pmt_dialogue(season, week_num)
    print(f"📝 Script ready: {len(dialogue)} hilarious PMT-style dialogue segments!")

    temp_dir = Path(__file__).resolve().parent / "temp_audio_pmt"
    ...
    for idx, (speaker, raw_text) in enumerate(dialogue):
        ...
        # Conversational pacing with edge-tts
        comm = edge_tts.Communicate(clean_text, voice, rate="+3%", pitch="+0Hz")
        await comm.save(seg_file)
        audio_files.append(seg_file)

    master_mp3 = str(Path(__file__).resolve().parent / f"bfl_monday_night_prime_week_{week_num}_{season}_pmt.mp3")
    concat_list = temp_dir / "concat.txt"
    with open(concat_list, 'w') as f:
        for af in audio_files:
            f.write(f"file '{af}'\n")

    print(f"🎵 Stitching master PMT podcast MP3 via ffmpeg -> {master_mp3}...")
    ...
    print(f"🎉 Master PMT Episode Created! Runtime: {mins}m {secs}s -> {master_mp3}")

    if post_to_discord and os.getenv("DISCORD_WEBHOOK_PODCAST"):
        print("🚀 Uploading PMT Show to Discord #press-room-podcast Forum...")
        thread_title = f"🎙️ BFL Monday Night Prime: PMT Week {week_num} Review ({mins}m {secs}s)"

        with open(master_mp3, 'rb') as f:
            files = {'file': (f"bfl_monday_night_prime_week_{week_num}_{season}_pmt.mp3", f, 'audio/mpeg')}
            data = {
                'username': 'BFL Monday Night Prime (PMT Desk)',
                'avatar_url': 'https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/full/3918298.png',
                'thread_name': thread_title,
                'content': f"🎙️ **BFL MONDAY NIGHT PRIME: WEEK {week_num} PMT REVIEW ({season})**\n*Chris & Dave drop the hilarious, unfiltered, 8-minute post-game breakdown: Josh Allen's 39-pt bad beat, Xavier Worthy & A.J. Brown goose eggs, Sydney demolishing Luke-ose, Emelie's bench agony, and press room roasts!*\n\n⏱️ **Duration:** `{mins}m {secs}s`\n🎧 **Listen to the full episode below:** 👇"
            }
            resp = requests.post(os.getenv("DISCORD_WEBHOOK_PODCAST"), data=data, files=files, timeout=45)
            if resp.status_code in [200, 204]:
                print("🎉 SUCCESS! PMT Monday Night Prime Episode uploaded directly to Discord!")
            else:
...
if __name__ == "__main__":
    asyncio.run(produce_pmt_show(2025, 1))
